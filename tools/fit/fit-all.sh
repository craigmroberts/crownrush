...
#   ITERS=200 TARGET=0.88 tools/fit/fit-all.sh
#
...
ITERS="${ITERS:-150}"; TARGET="${TARGET:-0.86}"
if [ $# -gt 0 ]; then names="$*"; else names=$(ls tools/fit/refs 2>/dev/null | grep -v '^_' | sed -E 's/\.(png|jpg|jpeg)$//' | sort -u); fi
...
for who in $names; do
  ...
  echo "=== $who  ($ref) ==="
  "$BLENDER" -b -P tools/fit/fit.py -- "$who" "$ref" --iters "$ITERS" --target "$TARGET" 2>&1 | grep -E "^(start|iter|colours|complete|stalled|reference|Traceback|  File|[A-Za-z]*Error)"
