...
export class Hud {
  ...
  showInfo(d) {
    ...
    h.push(`<h2>${iconSvg('gold', 22)} Coins</h2><p>You carry ${d.coins.count} coins, each worth ${d.coins.value} score.${d.coins.nextValue ? ` At level ${d.coins.nextAt} each one is worth ${d.coins.nextValue}.` : ''} Every pad costs coins except crews, which are paid in archers. What you mine is not spent anywhere: sell it at the trade post \u2014 which pays less for the tenth of anything you sell in a day, and less again for the twentieth, until dawn.</p>`);
